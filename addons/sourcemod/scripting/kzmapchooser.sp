...
Handle g_OldMapList = INVALID_HANDLE;
...
public void OnPluginStart()
{
	...
	g_MapListTier = CreateArray(arraySize);
	g_MapListWhiteList = CreateArray(arraySize);
	...
	EngineVersion g_Version = GetEngineVersion();
	...
	AutoExecConfig(true, "KZ_Mapchooser");
...
{
	if (GetArraySize(g_MapListWhiteList))
	{
		SetupTimeleftTimer();
	}
}
...
public Action Timer_StartMapVote(Handle timer, Handle data)
{
	...
	if (!GetArraySize(g_MapListWhiteList) || !GetConVarBool(g_Cvar_EndOfMapVote) || g_MapVoteCompleted || g_HasVoteStarted)
	{
		g_WarningTimer = INVALID_HANDLE;
		return Plugin_Stop;
	}
	...
	{
		int timerLocation = GetConVarInt(g_Cvar_TimerLocation);
...
public void Event_TeamPlayWinPanel(Handle event, const char[] name, booldontBroadcast)
{
	...
	if(GetEventInt(event, "round_complete") == 1 || StrEqual(name, "arena_win_panel"))
	{
		...
		if (!GetArraySize(g_MapListWhiteList) || g_HasVoteStarted || g_MapVoteCompleted || !GetConVarBool(g_Cvar_EndOfMapVote))
		{
			return;
		}
...
public Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!GetArraySize(g_MapListWhiteList) || g_HasVoteStarted || g_MapVoteCompleted)
	{
		return;
	}
...
{
	if (!GetArraySize(g_MapListWhiteList) || g_Cvar_Fraglimit == INVALID_HANDLE || g_HasVoteStarted)
	{
		return;
	}
...
{
	if (!GetArraySize(g_MapListWhiteList) || g_ChangeMapInProgress || g_HasVoteStarted || (!force && ((when == MapChange_MapEnd && !GetConVarBool(g_Cvar_EndOfMapVote)) || g_MapVoteCompleted)))
	{
		return;
	}
...
stock AddMapItem(const String:map[], const String:displayName[])
{
	if (g_NativeVotes)
	{
