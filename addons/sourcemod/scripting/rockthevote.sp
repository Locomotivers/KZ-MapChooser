...
{
	name = "KZTimer Rock The Vote",
	author = "AlliedModders LLC & Ace, Infra",
	description = "Provides RTV Map Voting for KZTimer with support for Map Tier display",
...
public void db_selectPlayerData(int client)
{
	...
	Format(szQuery, sizeof(szQuery), "SELECT name, points FROM playerrank WHERE style = 0 AND points >= (SELECT points FROM playerrank WHERE steamid = '%s' AND style = 0) ORDER BY points;", szSteamID);
...
public Action Command_ForceRTV(int client, int args)
{
	if (!client)
	{
		LogError("[RockTheVote] Not Allowed");
		return Plugin_Handled;
	}
	LogError("[RockTheVote] Starting Force RTV");
	ShowActivity2(client, "[RTV] ", "Initiated Vote Map");
